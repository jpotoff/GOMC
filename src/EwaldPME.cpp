...
EwaldPME::~EwaldPME() {
  for (uint b = 0; b < BOX_TOTAL; b++) {
    ...
    if (potentialMesh && potentialMesh[b])
      delete[] potentialMesh[b];
    if (scratchMesh && scratchMesh[b])
      delete[] scratchMesh[b];
    if (S_delta && S_delta[b])
      fftw_free(S_delta[b]);
    ...
    if (scratchPlan[b])
      fftw_destroy_plan(scratchPlan[b]);
  }
  delete[] S_ref;
  delete[] greenFunc;
  delete[] chargeMesh;
...
void EwaldPME::UpdateVectorsAndRecipTerms(bool output) {
  for (uint b = 0; b < BOXES_WITH_U_NB; ++b) {
    ...
    if (output) {
      ...
      double req = ff.pmeGridSpacing[b];
      double actX = currentAxes.axis.Get(b).x / K[b][0];
      double actY = currentAxes.axis.Get(b).y / K[b][1];
      double actZ = currentAxes.axis.Get(b).z / K[b][2];

      int initX = (int)ceil(currentAxes.axis.Get(b).x / req);
      int initY = (int)ceil(currentAxes.axis.Get(b).y / req);
      int initZ = (int)ceil(currentAxes.axis.Get(b).z / req);

      if (K[b][0] != initX || K[b][1] != initY || K[b][2] != initZ) {
        printf("Info: Box %d PMEGridSpacing adjusted to highly composite "
               "numbers for FFT efficiency.\n",
               b);
        printf("      Effective PMEGridSpacing: %.4f x %.4f x %.4f (Requested: "
               "%.4f)\n",
               actX, actY, actZ, req);
      }
    }
...
void EwaldPME::BoxReciprocalSetup(uint box, XYZArray const &molCoords) {
  ...
  if (kChanged || fwdPlan[box] == nullptr) {
    ...
    fwdPlan[box] =
        fftw_plan_dft_r2c_3d(Kx, Ky, Kz, chargeMesh[box], S_trial[box],
                             FFTW_ESTIMATE | FFTW_PRESERVE_INPUT);
    bwdPlan[box] = fftw_plan_dft_c2r_3d(Kx, Ky, Kz, S_trial[box],
                                        potentialMesh[box], FFTW_ESTIMATE);
    scratchPlan[box] =
        fftw_plan_dft_r2c_3d(Kx, Ky, Kz, scratchMesh[box], S_delta[box],
                             FFTW_ESTIMATE | FFTW_PRESERVE_INPUT);

    K_allocated[box][0] = Kx;
    K_allocated[box][1] = Ky;
    K_allocated[box][2] = Kz;
  }
      ...
      for (int ix = 0; ix < pmeOrder; ++ix) {
        ...
        for (int iy = 0; iy < pmeOrder; ++iy) {
          ...
          for (int iz = 0; iz < pmeOrder; ++iz) {
            ...
            chargeMesh[box][(gx * Ky + gy) * Kz + gz] +=
                charge * wy * bs_z[pmeOrder - 1 - iz];
          }
        }
      }
    }
...
void EwaldPME::UpdateGreenFunction(uint box, const BoxDimensions &axes,
  ...
  double Lx = axes.axis.Get(box).x, Ly = axes.axis.Get(box).y,
         Lz = axes.axis.Get(box).z;
  double alpha = ff.alpha[box], pre = 1.0 / (4.0 * alpha * alpha),
         vol = Lx * Ly * Lz;
    ...
    for (int iy = 0; iy < Ky; ++iy) {
      ...
      for (int iz = 0; iz < halfKz; ++iz) {
        ...
        if (kSq == 0)
          ...
          gf_out[idx] = num::qqFact * (4.0 * M_PI / (vol * kSq)) *
                        exp(-pre * kSq) * bx[ix] * by[iy] * bz[iz];
      }
    }
  }
}
...
void EwaldPME::UpdateAtomInMesh(uint box, const double *charges, uint nAtoms,
  ...
  for (uint i = 0; i < nAtoms; ++i) {
    ...
    double sx = s.x / currentAxes.axis.Get(box).x,
           sy = s.y / currentAxes.axis.Get(box).y,
           sz = s.z / currentAxes.axis.Get(box).z;
    sx -= floor(sx);
    sy -= floor(sy);
    sz -= floor(sz);
    double ux = sx * Kx, uy = sy * Ky, uz = sz * Kz;
    ...
    bspline::EvalAll(pmeOrder, uy - floor(uy), bs_y);
    bspline::EvalAll(pmeOrder, uz - floor(uz), bs_z);
    int nx = (int)floor(ux) - pmeOrder + 1, ny = (int)floor(uy) - pmeOrder + 1,
        nz = (int)floor(uz) - pmeOrder + 1;
    for (int ix = 0; ix < pmeOrder; ++ix) {
      int gx = ((nx + ix) % Kx + Kx) % Kx;
      double wx = bs_x[pmeOrder - 1 - ix];
      for (int iy = 0; iy < pmeOrder; ++iy) {
...
double EwaldPME::InterpolatePotential(uint box, const XYZ &r) const {
  ...
  for (int ix = 0; ix < pmeOrder; ++ix) {
    ...
    for (int iy = 0; iy < pmeOrder; ++iy) {
      ...
      for (int iz = 0; iz < pmeOrder; ++iz) {
        int gz = ((nz + iz) % Kz + Kz) % Kz;
        pot += wy * bs_z[pmeOrder - 1 - iz] *
               potentialMesh[box][(gx * Ky + gy) * Kz + gz];
      }
    }
  }
  return pot;
    ...
    for (uint i = 0; i < nAtoms; ++i) {
      ...
      for (int ix = 0; ix < pmeOrder; ++ix) {
        ...
        for (int iy = 0; iy < pmeOrder; ++iy) {
          ...
          for (int iz = 0; iz < pmeOrder; ++iz) {
            int gz = ((nz + iz) % Kz + Kz) % Kz;
            scratchMesh[box][(gx * Ky + gy) * Kz + gz] +=
                chg * wy * bs_z[pmeOrder - 1 - iz];
          }
        }
      }
    }
...
void EwaldPME::RecipInit(uint box, BoxDimensions const &axes) {
  ...
  K_trial[box][0] =
      nextFFTW((int)ceil(axes.axis.Get(box).x / ff.pmeGridSpacing[box]));
  K_trial[box][1] =
      nextFFTW((int)ceil(axes.axis.Get(box).y / ff.pmeGridSpacing[box]));
  K_trial[box][2] =
      nextFFTW((int)ceil(axes.axis.Get(box).z / ff.pmeGridSpacing[box]));
}

void EwaldPME::UpdateRecipVec(uint box) {
  if (box >= BOXES_WITH_U_NB)
    return;
  K[box][0] = K_trial[box][0];
  K[box][1] = K_trial[box][1];
  K[box][2] = K_trial[box][2];
  int nk = K[box][0] * K[box][1] * (K[box][2] / 2 + 1);
  memcpy(S_ref[box], S_trial[box], sizeof(fftw_complex) * nk);
...
void EwaldPME::UpdateRecip(uint box) {
  if (box >= BOXES_WITH_U_NB || !pendingUpdate[box])
    return;
  if (forceFullUpdate[box]) {
    ...
    double exactEnergy = DeltaERecip(
        box, (cachedSignNew[box] != 0.0 ? &cachedNewCoords[box] : nullptr),
        cachedSignNew[box],
        (cachedSignOld[box] != 0.0 ? &cachedOldCoords[box] : nullptr),
        cachedSignOld[box], cachedAtomIndices[box].data(),
        cachedCharges[box].data(), cachedNAtoms[box], true);
...
void EwaldPME::exgMolCache() {
  for (uint b = 0; b < BOXES_WITH_U_NB; ++b) {
    if (S_ref && S_ref[b]) {
      ...
      // If K_allocated diverged during the trial, arrays were re-malloc'd to
      // trial dimensions.
      if (K_allocated[b][0] != K[b][0] || K_allocated[b][1] != K[b][1] ||
          K_allocated[b][2] != K[b][2]) {
        grid_corrupted = true;
      }

      // Even if K integer boundaries didn't change, BoxReciprocalSetup strictly
      // overwrites the real-space chargeMesh with trial coordinates. If volume
      // changed, chargeMesh is corrupted.
      if (trialAxes[b].volume[b] != currentAxes.volume[b] ||
          trialAxes[b].axis.Get(b).x != currentAxes.axis.Get(b).x) {
        grid_corrupted = true;
      }

      // Restore parameters immediately so if BoxReciprocalSetup activates,
      // it orchestrates the reconstruction mathematically identically to the
      // committed K targets.
      K_trial[b][0] = K[b][0];
      K_trial[b][1] = K[b][1];
      K_trial[b][2] = K[b][2];
...
void EwaldPME::BoxForceReciprocal(XYZArray const &molCoords,
  ...
  UpdatePotentialMesh(box);
  int Kx = K[box][0], Ky = K[box][1], Kz = K[box][2];
  double Lx = currentAxes.axis.Get(box).x, Ly = currentAxes.axis.Get(box).y,
         Lz = currentAxes.axis.Get(box).z;
  double factorX = (double)Kx / Lx, factorY = (double)Ky / Ly,
         factorZ = (double)Kz / Lz;
  MoleculeLookup::box_iterator thisMol = molLookup.BoxBegin(box);
  MoleculeLookup::box_iterator end = molLookup.BoxEnd(box);
  while (thisMol != end) {
    uint m = *thisMol;
    ...
    for (uint a = 0; a < kind.NumAtoms(); ++a) {
      ...
      if (particleHasNoCharge[globalIdx]) {
        atomForceRec.Set(globalIdx, 0, 0, 0);
        continue;
      }
      double charge = kind.AtomCharge(a) * lambda;
      XYZ r = molCoords.Get(globalIdx);
      XYZ s = currentAxes.TransformUnSlant(r, box);
      ...
      bspline::EvalAll(pmeOrder, ux - floor(ux), bs_x);
      bspline::EvalAllDeriv(pmeOrder, ux - floor(ux), dbs_x);
      bspline::EvalAll(pmeOrder, uy - floor(uy), bs_y);
      bspline::EvalAllDeriv(pmeOrder, uy - floor(uy), dbs_y);
      bspline::EvalAll(pmeOrder, uz - floor(uz), bs_z);
      bspline::EvalAllDeriv(pmeOrder, uz - floor(uz), dbs_z);
      int nx = (int)floor(ux) - pmeOrder + 1,
          ny = (int)floor(uy) - pmeOrder + 1,
          nz = (int)floor(uz) - pmeOrder + 1;
      ...
      for (int ix = 0; ix < pmeOrder; ++ix) {
        int gx = ((nx + ix) % Kx + Kx) % Kx;
        double wx = bs_x[pmeOrder - 1 - ix], dwx = dbs_x[pmeOrder - 1 - ix];
        for (int iy = 0; iy < pmeOrder; ++iy) {
          int gy = ((ny + iy) % Ky + Ky) % Ky;
          double wy = bs_y[pmeOrder - 1 - iy], dwy = dbs_y[pmeOrder - 1 - iy];
          for (int iz = 0; iz < pmeOrder; ++iz) {
            int gz = ((nz + iz) % Kz + Kz) % Kz;
            double wz = bs_z[pmeOrder - 1 - iz], dwz = dbs_z[pmeOrder - 1 - iz];
      ...
      XYZ f_atom(-charge * dVdux * factorX, -charge * dVduy * factorY,
                 -charge * dVduz * factorZ);
      atomForceRec.Set(globalIdx, f_atom);
      f_mol += f_atom;
    }
...
double EwaldPME::MolReciprocal(XYZArray const &molCoords, const uint molIndex,
  ...
  vector<uint> atomIndices(length);
  vector<double> charges(length);
  for (uint i = 0; i < length; ++i) {
    atomIndices[i] = startAtom + i;
    charges[i] = particleCharge[startAtom + i];
  }
  ...
  double dE = DeltaERecip(box, &molCoords, 1.0, &cachedOldCoords[box], -1.0,
                          atomIndices.data(), charges.data(), length, false);
...
double EwaldPME::SwapDestRecip(const cbmc::TrialMol &newMol, const uint box,
  ...
  if (box >= BOXES_WITH_U_NB)
    return 0.0;
  uint length = newMol.GetKind().NumAtoms();
  vector<uint> atomIndices(length);
  vector<double> charges(length);
  uint startAtom = mols.MolStart(molIndex);
  for (uint i = 0; i < length; ++i) {
    atomIndices[i] = startAtom + i;
    charges[i] = newMol.GetKind().AtomCharge(i);
  }
  double dE = DeltaERecip(box, &newMol.GetCoords(), 1.0, nullptr, 0.0,
                          atomIndices.data(), charges.data(), length, false);
...
double EwaldPME::SwapSourceRecip(const cbmc::TrialMol &oldMol, const uint box,
  ...
  for (uint i = 0; i < length; ++i) {
    atomIndices[i] = startAtom + i;
    charges[i] = oldMol.GetKind().AtomCharge(i);
  }
  double dE = DeltaERecip(box, nullptr, 0.0, &oldMol.GetCoords(), -1.0,
                          atomIndices.data(), charges.data(), length, false);
...
double EwaldPME::ChangeLambdaRecip(XYZArray const &molCoords,
  ...
  return DeltaERecip(box, &molCoords, sign_new, &molCoords, sign_old,
                     atomIndices.data(), charges.data(), length, false);
}
...
EwaldPME::MolExchangeReciprocal(const std::vector<cbmc::TrialMol> &newMol,
  ...
  for (uint i = 0; i < newMol.size(); ++i)
    dE += SwapDestRecip(newMol[i], newMol[i].GetBox(), molIndexNew[i]);
  for (uint i = 0; i < oldMol.size(); ++i)
    dE += SwapSourceRecip(oldMol[i], oldMol[i].GetBox(), molIndexOld[i]);
  return dE;
}
...
Virial EwaldPME::VirialReciprocal(Virial &virial, uint box) const {
  if (box >= BOXES_WITH_U_NB || S_ref[box] == nullptr)
    return virial;
  SumMeshEnergy(box, S_ref[box], &virial, false);
  return virial;
}
