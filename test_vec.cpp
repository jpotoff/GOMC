...
int main() {
    ...
    std::vector<double> out1(10000000);
    std::vector<double> out2(10000000);
    std::vector<double> out3(10000000);
    ...
    #pragma omp simd
    for (size_t i = 0; i < vals.size(); ++i) out1[i] = num::erfc_cody(vals[i]);
    auto end1 = std::chrono::high_resolution_clock::now();
    
    auto start2 = std::chrono::high_resolution_clock::now();
    #pragma omp simd
    for (size_t i = 0; i < vals.size(); ++i) out2[i] = std::erfc(vals[i]);
    auto end2 = std::chrono::high_resolution_clock::now();
    
    auto start3 = std::chrono::high_resolution_clock::now();
    #pragma omp simd
    for (size_t i = 0; i < vals.size(); ++i) out3[i] = erfc_estrin(vals[i]);
    auto end3 = std::chrono::high_resolution_clock::now();
    
    double sum1 = 0, sum2 = 0, sum3 = 0;
    for (double v : out1) sum1 += v;
    for (double v : out2) sum2 += v;
    for (double v : out3) sum3 += v;
